...
function Solution({ header, href, text }) {
    return (
        <Link href={href} className='row-solution border-b-gray w-full flex justify-between py-16 px-4 max-lg:flex-col max-lg:space-y-6'>
            <h2 className='text-white text-2xl font-light'>{header}</h2>
            <p className='text-white-translucent w-1/2 text-xl leading-relaxed max-lg:w-full'>{text}</p>
        </Link>
    )
}